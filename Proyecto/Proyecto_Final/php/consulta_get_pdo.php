...
try {
    ...
    // Filtro estricto por clave de estación seleccionada (ej. ACO, MER, etc.)
    if ($estacion !== '') {
        $query .= " AND m.clave_estacion = :estacion";
    ...
    // Descarga en formato KML si se requiere
    if ($formato === 'kml') {
        ...
        foreach ($data as $row) {
            $nombre = htmlspecialchars($row['Nombre'] ?? 'Estación', ENT_XML1, 'UTF-8');
            $clave = htmlspecialchars($row['clave_estacion'] ?? '', ENT_XML1, 'UTF-8');
            $valor = htmlspecialchars((string) ($row[$gas] ?? ''), ENT_XML1, 'UTF-8');
    ...
    // Respuesta limpia en JSON para JavaScript
    echo json_encode([
        'total' => count($data),
        'datos' => $data
